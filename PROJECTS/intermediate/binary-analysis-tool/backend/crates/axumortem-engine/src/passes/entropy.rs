...
fn analyze_entropy(data: &[u8], sections: &[SectionInfo], entry_point: u64) -> EntropyResult {
    ...
    for section in sections {
        ...
        if let Some(packer) = detect_packer_by_section(&section.name) {
...
mod tests {
    ...
    fn section_flags_high_entropy() {
        ...
        let result = analyze_entropy(&data, &sections, 0x1000);
    ...
    fn packer_detection_by_section_name() {
        ...
        let result = analyze_entropy(&data, &sections, 0x11000);
